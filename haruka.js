...
const starts = async (haruka = new WAConnection()) => {
    ...
    haruka.browserDescription = [ 'Rifai Namikaze', 'Chrome', '3.0' ]
	CFonts.say('Rifai', {
		font: 'block',
    	color: ['#ff9c00'],
    	align: 'center',
		})
	CFonts.say(`Bot WhatsApp By RifaiChump`, {
		font: 'console',
		align: 'center',
	...
	haruka.on('group-participants-update', async (anu) => {
		...
		try {
						...
						if (anu.action == 'add' && !num.includes(haruka.user.jid)) {
							...
							try{
							await sendButLoc(mdata.id, `Goodbye @${num.split('@')[0]}\n⌯ָ   ֙Leave from group:\n${mdata.subject}` + '\n' + lang.leave(), `-1 member grup`,goodbye, [{"buttonId": `.owner`,"buttonText": {"displayText": "Owner"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							} catch {
								await sendButLoc(mdata.id, `Goodbye @${num.split('@')[0]}\n⌯ָ   ֙Leave from group:\n${mdata.subject}` + '\n' + lang.leave(), `-1 member grup`,ppmem2, [{"buttonId": `.owner`,"buttonText": {"displayText": "Owner"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							}
